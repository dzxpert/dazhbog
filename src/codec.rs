...
#[derive(Debug)]
pub enum CodecError { Short, Malformed(#[allow(dead_code)] &'static str) }
...
pub fn get_str_max(src: &mut &[u8], max_len: usize) -> Result<String, CodecError> {
    if src.len() < 4 { return Err(CodecError::Short); }
    let len = u32::from_le_bytes(src[0..4].try_into().unwrap()) as usize;
    if len > max_len { return Err(CodecError::Malformed("string too large")); }
    *src = &src[4..];
    if src.len() < len { return Err(CodecError::Short); }
    let s = std::str::from_utf8(&src[..len]).map_err(|_| CodecError::Malformed("utf8"))?;
    *src = &src[len..];
...
pub fn get_bytes_max(src: &mut &[u8], max_len: usize) -> Result<Vec<u8>, CodecError> {
    if src.len() < 4 { return Err(CodecError::Short); }
    let len = u32::from_le_bytes(src[0..4].try_into().unwrap()) as usize;
    if len > max_len { return Err(CodecError::Malformed("bytes too large")); }
    *src = &src[4..];
    if src.len() < len { return Err(CodecError::Short); }
